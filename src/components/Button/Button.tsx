...
const Button: React.FC<IButtonProps> = ({className, disable, onClick, value, type, actionType, to, iconType}) => {

    const button = () => (
        <button disabled={disable} className={`button ${`${'button__'+actionType}` || ''} ${type || ''} ${className || ''} ${iconType === "next" ? 'button__icon-animate' : ''}`} onClick={onClick}>
            {iconType === "next" &&
            <>
